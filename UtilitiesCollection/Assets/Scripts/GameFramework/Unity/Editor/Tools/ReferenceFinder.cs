namespace GameFramework.Unity.Editor.Tools
{
    using System.IO;
    using UnityEngine;
    using UnityEditor;
    ...
    public class ReferenceFinder : EditorWindow
    {
        ...
        void FindObjectReferences(Object toFind)
        {
            ...
            Debug.Log(string.Format("Find ref of {0}", AssetDatabase.GetAssetPath(toFind)), toFind);
            foreach (var go in m_references)
            {
                IterateChildren(go.transform.root, toFind);
            }
        ...
        private static void IterateChildren(Transform trans, Object toFind)
        {
            ...
            foreach (Component c in coms)
            {
                ...
                while (sp.NextVisible(true))
                {
                    ...
                    {
                        if (sp.objectReferenceValue == toFind)
                        {
                            ShowLog(trans.gameObject, c.GetType().Name, ObjectNames.NicifyVariableName(sp.name));
                        }
                    }
                }
            }
        ...
        private static void ShowLog(GameObject go, string componentName, string propertyName)
        {
            var str = "Ref in: {0}. Component: {1}, Property: {2}";
            Debug.Log(string.Format(str, GetFullPath(go), componentName, propertyName), go);
        }
